...
class UserPortalController extends Controller
{
    ...
    public function storeBooking(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            if (! $this->asBool($available)) return back()->withInput()->withErrors(['start_time' => 'Slot masa ini telah ditempah. Sila pilih masa lain.']);

            $canBook = $this->supabase->rpc($this->token(), 'can_user_book_today', ['p_booking_date' => $data['booking_date']]);
            if (! $this->asBool($canBook)) return back()->withInput()->withErrors(['booking_date' => 'Anda sudah mempunyai tempahan aktif pada tarikh ini.']);

            $this->supabase->insert($this->token(), 'bookings', $data + [
                'user_id' => session('profile.id'), 'status' => 'pending',
            ]);
            return redirect()->route('user.bookings')->with('success', 'Tempahan berjaya dihantar untuk kelulusan admin.');
        } catch (Throwable $e) {
            report($e);
    ...
    public function cancelBooking(string $booking): RedirectResponse
    {
        try {
            ...
            if (($record['status'] ?? '') !== 'pending') return back()->withErrors(['action' => 'Hanya tempahan yang masih menunggu boleh dibatalkan.']);
            ...
            report($e); return back()->withErrors(['action' => $e->getMessage()]);
        }
    }
    ...
    public function storeComplaint(Request $request): RedirectResponse
    {
        ...
        try {
            if (! empty($data['booking_id'])) $this->ownedBooking($data['booking_id']);
            $this->supabase->insert($this->token(), 'complaints', $data + ['user_id' => session('profile.id'), 'status' => 'open']);
            return back()->with('success', 'Aduan anda telah dihantar kepada admin.');
        } catch (Throwable $e) {
            report($e); return back()->withInput()->withErrors(['action' => $e->getMessage()]);
        }
    }
    ...
    public function checkout(string $booking): View|RedirectResponse
    {
        try {
            $record = $this->ownedBooking($booking);
            if (($record['status'] ?? '') !== 'approved') return redirect()->route('user.bookings')->withErrors(['action' => 'Checkout hanya tersedia untuk tempahan yang diluluskan.']);
            return view('user.checkout', ['booking' => $record]);
        } catch (Throwable $e) { return redirect()->route('user.bookings')->withErrors(['action' => $e->getMessage()]); }
    }
    ...
    public function storeCheckout(Request $request, string $booking): RedirectResponse
    {
        ...
        try {
            $record = $this->ownedBooking($booking);
            if (($record['status'] ?? '') !== 'approved') throw new \RuntimeException('Tempahan ini tidak boleh di-checkout.');
            ...
            return redirect()->route('user.bookings')->with('success', 'Checkout selesai. Terima kasih menjaga kebersihan dapur!');
        } catch (Throwable $e) {
            report($e); return back()->withInput()->withErrors(['action' => $e->getMessage()]);
        }
    }

    public function profile(): View { return view('user.profile'); }

    private function myBookings(): array { return $this->safeSelect('bookings', ['select' => '*', 'user_id' => 'eq.'.session('profile.id'), 'order' => 'created_at.desc']); }
    ...
    {
        try { return [$this->supabase->select($this->token(), $table, $query), null]; }
        catch (Throwable $e) { report($e); return [[], $e->getMessage()]; }
    }
    private function token(): string { return (string) session('supabase_token'); }
    private function asBool(mixed $value): bool
    {
        if (is_bool($value)) return $value;
        if (is_array($value) && count($value)) return $this->asBool(reset($value));
