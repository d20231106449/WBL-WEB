...
@section('title', 'Aduan')
@section('page-title', 'Aduan & Maklum Balas')
@section('content')
...
@forelse($complaints as $complaint)
    ...
    <article class="complaint-card">
        ...
        @if(!empty($complaint['admin_reply']))<div class="admin-reply"><span>Jawapan admin</span><p>{{ $complaint['admin_reply'] }}</p></div>@endif
        @if(($complaint['status'] ?? '') === 'open')<button class="action-button full" data-modal-open="complaint-{{ $complaint['id'] }}">Balas aduan</button>
        <dialog class="action-modal" id="complaint-{{ $complaint['id'] }}"><form method="POST" action="{{ route('admin.complaints.resolve', $complaint['id']) }}">@csrf @method('PATCH')<div class="modal-head"><div><p class="eyebrow">MAKLUM BALAS</p><h3>Jawab aduan pelajar</h3></div><button type="button" data-modal-close>×</button></div><div class="modal-summary"><p>{{ $complaint['complaint_text'] }}</p></div><label>Jawapan admin<textarea name="admin_reply" rows="5" required placeholder="Tulis jawapan yang jelas dan membantu…"></textarea></label><div class="modal-actions"><button type="button" class="button ghost" data-modal-close>Batal</button><button class="button primary">Hantar & selesaikan</button></div></form></dialog>@endif
    </article>
