...
<form method="POST" action="{{ route('user.complaints.store', [], false) }}" class="user-form-card complaint-form">@csrf
    ...
    <button class="user-primary-button" type="submit">Hantar aduan <span>&rarr;</span></button>
</form>
<section class="complaint-history"><div class="user-section-head"><div><p>SEJARAH</p><h2>Aduan anda</h2></div></div>
@forelse($complaints as $complaint)<article class="user-complaint-card"><div>@include('partials.status',['status'=>$complaint['status']])<time>{{ \Illuminate\Support\Carbon::parse($complaint['created_at'])->translatedFormat('d M Y') }}</time></div><p>&ldquo;{{ $complaint['complaint_text'] }}&rdquo;</p>@if(!empty($complaint['admin_reply']))<aside><strong>Jawapan pentadbir</strong><p>{{ $complaint['admin_reply'] }}</p></aside>@endif</article>@empty<div class="user-empty compact"><span>&#9671;</span><p>Belum ada aduan dihantar.</p></div>@endforelse</section>
</div>
@endsection
