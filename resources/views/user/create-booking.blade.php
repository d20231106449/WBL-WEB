...
@section('content')
<div class="user-page-title"><a href="{{ route('user.dashboard') }}">←</a><div><p>TEMPAHAN BAHARU</p><h1>Pilih waktu anda</h1><span>Setiap tempahan berlangsung selama satu jam.</span></div></div>
<div class="booking-form-grid">
<form class="user-form-card" method="POST" action="{{ route('user.bookings.store') }}" data-booking-form data-server-now="{{ now()->toIso8601String() }}">@csrf
    ...
    <div class="time-slots">
        ...
        <label><input type="radio" name="start_time" value="{{ $time }}" data-booking-time {{ old('start_time')===$time ? 'checked':'' }} required><span>{{ \Illuminate\Support\Carbon::createFromFormat('H:i',$time)->format('g:i A') }}</span></label>
        @endforeach
    </div>
    @error('start_time')<span class="field-error">{{ $message }}</span>@enderror
    <div class="form-section-title"><span>3</span><div><strong>Maklumat penggunaan</strong><small>Bantu pentadbir memahami tujuan anda</small></div></div>
    <div class="form-two"><label>Bilangan pengguna<input class="@error('pax') is-invalid @enderror" type="number" name="pax" value="{{ old('pax',1) }}" min="1" max="30" required>@error('pax')<span class="field-error">{{ $message }}</span>@enderror</label><label>Tujuan<select class="@error('purpose') is-invalid @enderror" name="purpose" required><option value="">Pilih tujuan</option>@foreach(['Memasak','Program kolej','Aktiviti persatuan','Lain-lain'] as $purpose)<option {{ old('purpose')===$purpose?'selected':'' }}>{{ $purpose }}</option>@endforeach</select>@error('purpose')<span class="field-error">{{ $message }}</span>@enderror</label></div>
    <button class="user-primary-button" type="submit">Hantar kepada pentadbir untuk kelulusan <span>→</span></button>
</form>
